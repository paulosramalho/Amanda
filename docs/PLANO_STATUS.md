...
| 2. Banco e Prisma | Concluido (codigo) | 08/04/2026 | Prisma 6.17.1 integrado (`@prisma/client` + `prisma`), schema criado, migration versionada em `backend/prisma/migrations`, scripts `prisma:generate`, `prisma:migrate:deploy`, `prisma:migrate:dev` e `postinstall` configurados, `start` aplicando migration automaticamente. | Validar deploy no Render apos push (execucao real de migration no Neon). | Imediato apos push |
...
| 5. API de negocio | Em andamento (base) | 08/04/2026 | Endpoints base prontos: `/`, `/health`, `/health/db`, `/business-date`. Regra de data de negocio aplicada no backend. | Criar endpoints de leads e dashboard com filtros por periodo/canal e agregacoes. | 09/04/2026 |
...
| 7. Frontend dedicado | Pendente | - | Vercel provisionado para o projeto. | Criar painel web e conectar no backend Render. | 10/04/2026 |
